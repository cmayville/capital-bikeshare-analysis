...
```{r}
# but let's select the relevant deviations in a more principled way, accounting for multiple testing.
...
data_permuted <- lapply(data_list, function(df) {
  df %>% mutate(duration = sample(length(duration)))
}) %>% bind_rows()
# should repeat above for n permutations, but don't do this yet for speed reasons
```

```{r}
null_routemonth <- data_permuted %>% group_by(route,month) %>% summarise(mean_duration = mean(duration))
obs_routemonth <- data %>% group_by(route,month) %>% summarise(mean_duration = mean(duration))
hist(log10(null_routemonth$mean_duration),col='grey',main="null vs observed mean month-route durations",xlim=c(0,5),ylim=c(0,30000))
...
```
```{r}
hist(obs_routemonth$mean_duration, breaks=1000, freq=FALSE,col='grey',xlim=c(0,5000))
hist(null_routemonth$mean_duration,breaks=20,add=TRUE,freq=FALSE,col=rgb(red=1,green=0,blue=0,alpha=0.2),main="null vs observed mean month-route durations")
legend("topright",legend=c('observed','null'),fill=c('grey',rgb(red=1,green=0,blue=0,alpha=0.2)))
```

```{r}
hist(obs_routemonth$mean_duration,col=rgb(red=1,green=0,blue=0,alpha=0.2))
```
