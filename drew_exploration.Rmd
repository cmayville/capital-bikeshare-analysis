...
data_by_route <- data %>% group_by(route) %>% summarise(mean_route_duration = mean(duration))
data<-left_join(data,data_by_route,by='route')
data['demeaned_duration'] <- data$demeaned_duration-data$mean_route_duration
...
barplot(fe_bonf_bymonth$prop_discoveries_bonf,names.arg=fe_bonf_bymonth$months_since_Jan_2010,xlab='months after Jan 2010',ylab = 'proportion of routes with discovery')
barplot(fe_bonf_bymonth$prop_discoveries_bh,names.arg=fe_bonf_bymonth$months_since_Jan_2010,xlab='months after Jan 2010',ylab = 'proportion of routes with discovery')

perm_bonf_bymonth <- compare_obs_null %>% group_by(months_since_Jan_2010) %>% summarise(
  prop_discoveries_bonf = sum(discoveries_bonf,na.rm=TRUE)/n(),
  prop_discoveries_bh = sum(discoveries_hb,na.rm=TRUE)/n()) %>% arrange(as.integer(months_since_Jan_2010))
barplot(perm_bonf_bymonth$prop_discoveries_bonf,names.arg=perm_bonf_bymonth$months_since_Jan_2010,xlab='months after Jan 2010',ylab = 'proportion of routes with discovery')
barplot(perm_bonf_bymonth$prop_discoveries_bh,names.arg=perm_bonf_bymonth$months_since_Jan_2010,xlab='months after Jan 2010',ylab = 'proportion of routes with discovery')
...
barplot(fe_bonf_months$mean_discoveries,names.arg = fe_bonf_months$n_months)
plot(fe_bonf$route_n,fe_bonf$n_discoveries)

fe_hb_months <- fe_hb %>% group_by(n_months) %>% summarise(mean_discoveries = mean(n_discoveries))
barplot(fe_hb_months$mean_discoveries,names.arg = fe_hb_months$n_months)
plot(fe_hb$route_n,fe_hb$n_discoveries)

perm_bonf_months <- perm_bonf %>% group_by(n_months) %>% summarise(mean_discoveries = mean(n_discoveries))
barplot(perm_bonf_months$mean_discoveries,names.arg = perm_bonf_months$n_months)
plot(perm_bonf$route_n,perm_bonf$n_discoveries)

perm_hb_months <- perm_hb %>% group_by(n_months) %>% summarise(mean_discoveries = mean(n_discoveries))
barplot(perm_hb_months$mean_discoveries,names.arg = perm_hb_months$n_months)
plot(perm_hb$route_n,perm_hb$n_discoveries)
```
